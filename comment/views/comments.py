...
class CreateComment(CanCreateMixin, CommentCreateMixin):
    ...
    def form_valid(self, form):
        ...
        # sending email to the article author
        if author_email:
            email = EmailMessage("دیدگاه جدید", "دیدگاه جدیدی برای مقاله «{}» که شما نویسنده آن هستید اضافه شد:\n{}{}"
                                 .format(article, current_site, reverse('myapp:article_detail', kwargs={'slug':article.slug})), 
                                 to=[author_email])
            email.send() 

        # sending email to the comment author
        if user_email:
            email = EmailMessage("دیدگاه دریافت شد", "ممنون که به مقاله نظر دادید.", 
                                 to=[user_email])
            email.send()

        # sending email to the parent comment author
        if parent_email:
            email = EmailMessage("پاسخ به دیدگاه شما", "پاسخی به دیدگاه شمادر مقاله «{}» ثبت شده است برای مشاهده در لینک زیر کلیک کنید:\n{}{}"
                                 .format(article, current_site, reverse('myapp:article_detail', kwargs={'slug':article.slug})), 
                                 to=[parent_email])
            email.send() 
